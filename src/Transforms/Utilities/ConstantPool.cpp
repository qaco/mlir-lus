...
namespace mlir {
  ...
  Value ConstantPool::getVector(std::vector<bool> data, Type t) {
    ...
    arith::ConstantOp::build(myBuilder, state, vectorAttr);
    Operation* constantOpPtr = myBuilder.createOperation(state);
    arith::ConstantOp constantOp = dyn_cast<arith::ConstantOp>(constantOpPtr);
  ...
  Value ConstantPool::buildInt(Type type, long value) {
    ...
    OperationState state(myBuilder.getUnknownLoc(),
			 arith::ConstantOp::getOperationName());
    arith::ConstantOp::build(myBuilder, state, attr);
    Operation* constantOpPtr = myBuilder.createOperation(state);
    arith::ConstantOp constantOp = dyn_cast<arith::ConstantOp>(constantOpPtr);
  ...
  Value ConstantPool::increment(OpBuilder &builder, Value v) {
    ...
    arith::AddIOp::build(builder, addState, v, one);
    Operation* addOpPtr = builder.createOperation(addState);
    arith::AddIOp addOp = dyn_cast<arith::AddIOp>(addOpPtr);
  ...
  Value ConstantPool::castToBool(OpBuilder& builder, Value v) {
    ...
    OperationState state(v.getLoc(), arith::CmpIOp::getOperationName());
    arith::CmpIOp::build(builder, state, builder.getI1Type(),
			 arith::CmpIPredicate::ne, v, zero);
    Operation *cmpOpPtr = builder.createOperation(state);
    arith::CmpIOp cmpOp = dyn_cast<arith::CmpIOp>(cmpOpPtr);
  ...
  Value ConstantPool::castToIndex(OpBuilder &builder, Value v) {
    if (v.getType() == myBuilder.getIndexType()) {
      ...
      arith::IndexCastOp::build(builder, castState, builder.getIndexType(), v);
      Operation *indexCastOpPtr = builder.createOperation(castState);
      arith::IndexCastOp indexCastOp = dyn_cast<arith::IndexCastOp>(indexCastOpPtr);
