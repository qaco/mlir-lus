...
namespace mlir {
  namespace lus {
    ...
    void NodeOp::build(Builder &builder, OperationState &state,
      ...
      Region *body_dom = state.addRegion();
      Region *active;
      if (regionKind == RegionKind::Graph)
	active = body;
      else
	active = body_dom;

      for (auto i: inputTypes)
	active->addArgument(i);
	...
	if(!succeeded(ca.analyseLogical())) return failure() ;
      }
    ...
    ArrayRef<StringRef> NodeOp::getAttributeNames() {
      static StringRef attrNames[] = { getTypeAttrName() };
