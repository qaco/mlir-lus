...
namespace mlir {
  namespace lus {
    ...
    public:
      ...
      // class Path: public std::list<Edge> {
      // public:
      // 	static unsigned getHashValue(Path p) {
      // 	  unsigned seed = p.size();
      // 	  for(auto& i : p) {
      // 	    seed ^= i.getHashValue() + 0x9e3779b9 + (seed << 6) + (seed >> 2);
      // 	  }
      // 	  return seed;
      // 	}
      // };
	
      using Path = SmallVector<Edge,4>;
