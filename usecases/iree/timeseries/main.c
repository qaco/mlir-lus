...
iree_status_t rnn_reset(iree_runtime_session_t* session,
  ...
  const size_t state0_size = 3*100*sizeof(float);
  const size_t state1_size = 3*100*sizeof(float);
...
iree_status_t rnn_step(iree_runtime_session_t *session,
  ...
  size_t input_shape[3] = {1,3,1};
  const size_t input_elt_size = sizeof(float);
  status = load_next_input(status, session, &call, input_shape, 3, input_elt_size, input);
  ...
  const size_t output_size = 3*1*sizeof(float);
