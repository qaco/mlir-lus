...
void task1_start(TASK_ID_TYPE tid) {
  ...
  model(tid, sched_read_input, sched_write_output);
}

void tick_task(TASK_ID_TYPE tid) {
  for(;;) {
    struct memref_2d_f32 to_task ;
    struct memref_2d_f32 from_task ;
    alloc_memref_2d_f32(&to_task,1,40) ;
    alloc_memref_2d_f32(&from_task,1,4) ;
    float* aligned = (float*)to_task.aligned ;
    for(int i=0;i<2;i++)aligned[i] = (float)i ;
