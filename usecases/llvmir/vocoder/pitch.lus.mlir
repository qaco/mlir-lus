...
//   // Write 256 samples from %output_acc to the soundcard
//   %out = call @extract_samples(%sample) : (tensor<1024xf32>)->(tensor<256xi16>)
//   %sndbufout = call @mono2stereo(%out):(tensor<256xi16>)->(tensor<512xi16>)
...
// 		     %sample:tensor<1024xf32>)
// 		 ->(tensor<1024xf32>) {  // %output_acc
//   lus.yield(%sample: tensor<1024xf32>)
// }
...
lus.node @kbd_ctrl(%kbd:i8)->(i1,f32,f32) {
  ...
  %cha = arith.constant  97 : i8 // Letter 'a'
  %chs = arith.constant 115 : i8 // Letter 's'
  %chn = arith.constant 110 : i8 // Letter 'n'
  %chm = arith.constant 109 : i8 // Letter 'm'
  %pls = arith.constant  43 : i8 // Character '+'
  %min = arith.constant  45 : i8 // Character '-'
  %ca = arith.cmpi "eq",%kbd,%cha : i8
  %cs = arith.cmpi "eq",%kbd,%chs : i8
  %cn = arith.cmpi "eq",%kbd,%chn : i8
  %cm = arith.cmpi "eq",%kbd,%chm : i8
  %cpls = arith.cmpi "eq",%kbd,%pls : i8
  %cmin = arith.cmpi "eq",%kbd,%min : i8
  ...
  %f0 = arith.constant 0.0 : f32 // Min value
  %f1 = arith.constant 1.0 : f32 // Max value
  %fip = arith.constant 0.1 : f32 // Positive increment
  %fim = arith.constant -0.1 : f32 // Negative increment
  %volume = lus.fby %f1 %pre_volume : f32
  %y = select %cpls,%fip,%f0 : f32
  %inc = select %cmin,%fim,%y: f32
  %vol_tmp1 = arith.addf %volume,%inc : f32
  %pre_volume = math.abs %vol_tmp1 : f32
  // Computing the pitch shift in number of semitones
  %f3 = arith.constant 3.0 : f32 // Max value
  %fcp = arith.constant 0.5 : f32 // Positive pitch increment
  %fcm = arith.constant -0.5 : f32 // Negative pitch increment
  %semitones = lus.fby %f3 %pre_semitones :f32
  %z = select %cn,%fcp,%f0 : f32
  %inc_semitones = select %cm,%fcm,%z: f32
  %pre_semitones = arith.addf %semitones,%inc_semitones : f32
