...
func @compute_twiddles(%res:memref<?xcomplex<f32>>){
  ...
  %sz2 = arith.divsi %size,%2 : index
  scf.while (%span = %1) : (index) -> (index) {
    // while %span <= (%size/2)
    %cond = arith.cmpi "sle",%span,%sz2 : index
    scf.condition(%cond) %span : index
  } do {
  ^bb0(%span: index):
    %spani = arith.index_cast %span:index to i32
    %spanf = arith.sitofp %spani:i32 to f32
    %x1 = arith.divf %pi,%spanf : f32
    %primitive_root = arith.subf %f0, %x1 : f32 
    scf.for %i = %0 to %span step %1 {
      // Compute the complex twiddle value
      %ii = arith.index_cast %i:index to i32
      %if = arith.sitofp %ii:i32 to f32
      %x2 = arith.mulf %if, %primitive_root : f32
      %re = math.cos %x2 : f32
      %im = math.sin %x2 : f32
      %twiddle = complex.create %re, %im : complex<f32>
      // Compute the index %span+%i
      %idx = arith.addi %span, %i : index
      memref.store %twiddle, %res[%idx] : memref<?xcomplex<f32>>
    }
    // %span := %span*2
    %span_next = arith.muli %span, %2 : index
...
func @fft_normalize(%data:memref<?xcomplex<f32>>,%res:memref<?xcomplex<f32>>) {
  ...
  %tmp1 = arith.index_cast %size:index to i32
  %tmp2= arith.sitofp %tmp1:i32 to f32
  %f1 = arith.constant 1.0 : f32
  %norm_val = arith.divf %f1, %tmp2 : f32
...
//    assert %evensize, "@fft: size of input data is not power of 2."
    ...
    %cond = arith.cmpi "sge",%curr_size,%2 : index
    scf.condition(%cond) %curr_size : index
  } do {
  ^bb0(%curr_size: index):
    // Half the current size
    %halfsize = arith.divsi %curr_size,%2 : index
    ...
    %iter_no = arith.divsi %size, %curr_size : index
    scf.for %iter = %0 to %iter_no step %1 {
      // The input data is in %tmp, I will write it to %res
      // Extract the data half-vectors
      %base0 = arith.muli %iter, %curr_size : index
      %base1 = arith.addi %base0, %halfsize : index
...
func @ifft(%perm:memref<?xi32>,%twid:memref<?xcomplex<f32>>,%data:memref<?xcomplex<f32>>,%res:memref<?xcomplex<f32>>) {
  ...
  %tmp1 = arith.index_cast %size:index to i32
  %norm_val= arith.sitofp %tmp1:i32 to f32
  scf.for %idx = %0 to %size step %1 {
